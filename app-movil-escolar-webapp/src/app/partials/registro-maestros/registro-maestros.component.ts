...
export class RegistroMaestrosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.activatedRoute.snapshot.params['id'] != undefined){
      ...
      this.maestro.rol = 'maestro'; // Cambiar this.rol por 'maestro'
  ...
  public actualizar(){
    ...
    this.maestrosService.actualizarMaestro(this.maestro).subscribe(
      (response) => {
        ...
        this.token = this.facadeService.getSessionToken();
        if(this.token && this.token !== ""){
          this.router.navigate(["/maestros"]); // Asegúrate que esta ruta exista
        } else {
      ...
      (error) => {
        ...
        if(error.status === 401) {
          ...
          alert("Error al actualizar maestro: " + (error.error?.message || 'Error desconocido'));
        }
      }
    );
}
