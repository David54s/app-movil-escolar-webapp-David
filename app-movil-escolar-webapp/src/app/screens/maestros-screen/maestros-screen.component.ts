// maestros-screen.component.ts CON SORTING MEJORADO
...
export class MaestrosScreenComponent implements OnInit, AfterViewInit {
  ...
  displayedColumns: string[] = ['id_trabajador', 'nombre', 'apellido', 'email', 'fecha_nacimiento', 'telefono', 'rfc', 'cubiculo', 'area_investigacion', 'editar', 'eliminar'];
        ...
        if (this.lista_maestros.length > 0) {
          ...
          setTimeout(() => {
            ...
            this.dataSource.filterPredicate = (data: DatosUsuario, filter: string) => {
              ...
              return data.id_trabajador.toString().includes(searchStr) ||
                     fullName.includes(searchStr) ||
                     data.email.toLowerCase().includes(searchStr) ||
                     data.telefono.includes(searchStr) ||
                     data.rfc.toLowerCase().includes(searchStr) ||
                     data.cubiculo.toString().includes(searchStr);
            };
          });
        }
      }, (error) => {
